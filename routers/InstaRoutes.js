...
function getVideoLinkFromHtml(html) {
    ...
    // console.log(crop);
    // console.log(JSON.parse(crop).video_url);
    return JSON.parse(crop).video_url;
}

router.get('/', async (req, res)=>{
    try{
        const instagramLink = "https://www.instagram.com/p/CR75ljoJhG-/?utm_medium=copy_link";
        const dlink=await getPostLink(instagramLink);
        
        res.status(200).json(dlink.link);
    }catch(err){
